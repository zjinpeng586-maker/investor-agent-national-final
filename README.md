...
| 数据中心只读 | `local` 使用本地持久库；`public` 提供在线会话独立导入；`readonly` 明确禁止写入。 |
...
`.xls` 由 `xlrd` 读取，`.xlsx` 由 `openpyxl` 读取。旧 `reset_db.py` 已禁用自动删除；需要新的资料位置时使用新数据目录。`run_local_network.bat` 是额外的局域网 **只读访问** 入口，设置 `FINANCIAL_DEPLOYMENT=readonly`，不提供上传和修改权限；不要把可写本地模式直接绑定公网。
...
| 上传按钮只读 | `local` 使用本地持久库；`public` 提供在线会话独立导入；`readonly` 明确禁止写入。 |
...
2026-09-25 归因树封板基线：Python 3.12 全量测试 **448 项通过**，固定 Benchmark **55/55**。后续在线披露增强版的测试与公网实测见 [在线披露增强验收](docs/在线披露增强验收.md)，实现原理见 [技术说明书](docs/技术说明书.md)。固定评测结果不代表任意财报或全市场数据的泛化准确率。

内置数据用于功能体验，实际研究应核验原始披露与数据口径。扫描 PDF、复杂排版和模糊单位需人工核验。真实云端模型接口、公开披露网络和评审设备兼容性需按实际环境验证。

云端默认 `public` 模式使用每个浏览器会话独立的 SQLite 数据库、附件目录与检索索引。支持公开披露检索、下载解析、复核确认、真实入库，再由财报问数与企业分析读取同一会话的新数据。新访客不能读取其他会话资料，也不自动迁移服务器旧库。每会话最多保存20个文件、合计100 MB。刷新、断线或重启后不保证恢复，云端临时磁盘不提供长期持久化承诺，请自行保存原始文件。需要长期使用时采用本地持久库；账号与云端持久存储不在本版范围内。

本地启动脚本使用 `local`，继续保留原统一资料库。显式 `readonly` 模式禁止写入。评测始终使用内部隔离数据库，不读写业务资料。云端不应通过设置 `local` 来开放共享数据库。

离线问答新增“核心财务指标”“关键财务指标”“全部财务指标”“财务数据一览”“财务指标总览”，一次查询七项可用指标，并支持“那2023年呢”等连续追问；支持“净资产回报率”“经营净现金”“每股盈利”等别称。示例：`比亚迪2024年核心财务指标是多少？`。新导入企业和股票代码使用相同查询路径；缺失值明确提示，不补为零。
